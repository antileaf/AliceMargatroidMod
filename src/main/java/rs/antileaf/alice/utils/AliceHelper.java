...
public abstract class AliceHelper {
	public static String getModID() {
		return AliceMargatroidMod.SIMPLE_NAME;
	}
	
	public static String makeID(String name) {
		return "AliceMargatroid:" + name;
	}
...
//		}
	...
	public static String getCardImgFilePath(String name) {
		return AliceHelper.getImgFilePath("cards", name);
	}
	
	public static String getRelicImgFilePath(String name) {
		return AliceHelper.getImgFilePath("relics", name);
	}
	
	public static String getRelicOutlineImgFilePath(String name) {
		return AliceHelper.getImgFilePath("relics/outline", name);
	}
	
	public static String getRelicLargeImgFilePath(String name) {
		return AliceHelper.getImgFilePath("relics/large", name);
	}
	
	public static String getOrbImgFilePath(String name) {
		return AliceHelper.getImgFilePath("orbs", name);
	}
	
	public static String getPowerImgFilePath(String name) {
		return AliceHelper.getImgFilePath("powers", name);
	}
	
	public static String getEventImgFilePath(String name) {
		return AliceHelper.getImgFilePath("events", name);
	}
	
	public static String getLocalizationFilePath(String name) {
		return "AliceMargatroidMod/localization/" + AliceHelper.getLangShort() + "/" + name + ".json";
	}
	
	public static void loadCustomStrings(Class<?> clz, String name) {
		String lang = AliceHelper.getLangShort();
		
		String path = AliceHelper.getLocalizationFilePath(name);
...
//	}
	...
	public static void addCardToHand(AbstractCard card) {
		if (AbstractDungeon.player.hand.size() < BaseMod.MAX_HAND_SIZE)
			AliceHelper.addEffect(new ShowCardAndAddToHandEffect(
					card,
					Settings.WIDTH / 2.0F - (25.0F * Settings.scale + AbstractCard.IMG_WIDTH),
					Settings.HEIGHT / 2.0F));
		else
			AliceHelper.addEffect(new ShowCardAndAddToDiscardEffect(
					card,
					Settings.WIDTH / 2.0F + 25.0F * Settings.scale + AbstractCard.IMG_WIDTH,
					Settings.HEIGHT / 2.0F));
	}
	...
	public static void addActionsToTop(AbstractGameAction... actions) {
		for (AbstractGameAction action : AliceMiscHelper.reversedArray(actions))
...
//
	...
	public static String getDuplicateCardsMessage(ArrayList<AbstractCard> group, AbstractCard except) {
		...
		ArrayList<AbstractCard> duplicates = AliceHelper.duplicateCards(group, except);
