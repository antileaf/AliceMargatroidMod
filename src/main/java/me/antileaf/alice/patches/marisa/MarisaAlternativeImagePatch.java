package me.antileaf.alice.patches.marisa;
...
import me.antileaf.alice.cards.marisa.AbstractAliceMarisaCard;
import me.antileaf.alice.cards.marisa.AliceSpark;
...
//						new Matcher.ConstructorCallMatcher(MakeTempCardInHandAction.class));
		...
		public static SpireReturn<Void> Insert(DoubleSpark _inst, AbstractPlayer p, AbstractMonster m, AbstractCard c) {
			if ((c instanceof Spark) && AliceMarisaCardFields.isAliceMarisaCard.get(_inst)) {
				AbstractAliceMarisaCard.setImages((CustomCard) c, AliceSpark.SIMPLE_NAME);

				AliceHelper.addToBot(new AliceAddCardToHandAction(c));
