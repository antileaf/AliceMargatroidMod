...
import me.antileaf.alice.action.unique.KirisameMahoutenAction;
...
//	public static final String ID = AliceSpireKit.makeID(SIMPLE_NAME);
	...
	public void use(AbstractPlayer p, AbstractMonster m) {
		...
		for (int i = 0; i < this.magicNumber; i++) {
			...
			if (potion instanceof FruitJuice ||
					potions.stream().anyMatch(po -> potion.ID.equals(po.ID))) {
				i--;
				continue;
			}
				...
				(c) -> {
					if (c instanceof MarisasPotion) {
						...
						if (marisasPotion.potion != null) {
							...
							AliceHelper.log("KirisameMahouten: Potion is null.");
					}
					else
						AliceHelper.log("KirisameMahouten: Potion is not MarisasPotion.");
				},
